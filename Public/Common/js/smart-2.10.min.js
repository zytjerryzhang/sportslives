...
b.push({type:"build-in",name:"foreach",params:a,subTree:d,subTreeElse:e});(a=G("foreach\\s[^}]+","/foreach","foreachelse",c))?(n(c.slice(0,a.index),d),n(c.slice(a.index+a[0].length).replace(/^[\r\n]/,""),e)):n(c,d)},process:function(a,b){var c=r(a.params,b),d=c.from;if(d == null){d=[];}else if (!(d instanceof Object)){d=[d];}var e=S(d);b[c.item+"__total"]=e;"name"in c&&(b.smarty.foreach[c.name]={},b.smarty.foreach[c.name].total=e);var f="",g=0,h;for(h in d)if(d.hasOwnProperty(h)){if(b.smarty["break"])break;b[c.item+"__key"]=isNaN(h)?
