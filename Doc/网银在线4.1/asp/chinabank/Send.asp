...
'****************************************	
	...
	v_url = "http://localhost/chinabank/Receive.asp" ' 商户自定义返回接收支付结果的页面。对应Receive.asp示例。
													 ' 参照"网银在线支付B2C系统商户接口文档v4.1.doc"中2.3.3.1。

									
	key = "test"									 ' 参照"网银在线支付B2C系统商户接口文档v4.1.doc"中2.4.1进行设置。

	remark2 = "[url:=http://domain/chinabank/AutoReceive.asp]" ' 服务器异步通知的接收地址。对应AutoReceive.asp示例。必须要有[url:=]格式。
